...
if grep -qi "fedora" /etc/os-release; then
    ...
    sleep 2
    sudo yum -qq -y update && sudo yum -qq -y install git aria\2 bash xz wget curl pv jq python3-pip mediainfo procps-ng qbittor\rent-nox
    ...
    sleep 2
    sudo apt -y -qq update && sudo apt -y -qq install -y git wget curl aria\2 pv jq python3-pip mediainfo qbittor\rent-nox 
