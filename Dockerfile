...
RUN dnf -qq -y update && dnf -qq -y install git bash xz wget curl pv jq python3-pip mediainfo psmisc procps-ng && if [[ $(arch) == 'aarch64' ]]; then   dnf -qq -y install gcc python3-devel; fi && python3 -m pip install --upgrade pip setuptools
